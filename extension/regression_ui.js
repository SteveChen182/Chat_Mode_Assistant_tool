/**
 * GFX/GOP Driver Regression UI — Chat Mode Assistant adapter
 * ===========================================================
 * Adapted from Check-gfx-driver-regression/regression_ui.js for use in
 * the Chat Mode Assistant Chrome extension (port 8776).
 *
 * Defines all required globals before the shared regression logic runs.
 */

// ── Adapter globals ───────────────────────────────────────────────────────────
const BRIDGE_BASE_URL = "http://127.0.0.1:8776";

// isSending: local lock to prevent double-clicking regression buttons.
// (activeHsdId is defined by sidepanel.js which loads after this file)
let isSending = false;
let _lastRegBuildType = "gfx";  // updated on each check; drives History tab pre-selection

function t(key) {
  const _i18n = {
    qbLoginTitle:          "QuickBuild Login",
    qbLoginMsg:            "Enter your Intel credentials to search QuickBuild builds.",
    qbContinueBtn:         "Continue",
    qbCancelBtn:           "Cancel",
    qbProgressLogin:       "Logging in...",
    qbLoggingIn:           "Logging in to QB...",
    qbProgressSearch:      "Searching GFX builds...",
    qbGopProgressSearch:   "Searching GOP builds...",
    qbSearching:           "Searching QB...",
    qbProgressDone:        "Done",
    regressionTitle:       "GFX Driver Regression",
    gopRegressionTitle:    "GOP Driver Regression",
    regressionChecking:    "Checking GFX regression...",
    gopRegressionChecking: "Checking GOP regression...",
    regProgressHsd:        "Fetching HSD regression data...",
    regGopProgressHsd:     "Fetching HSD GOP regression data...",
    regProgressQbLogin:    "Connecting to QuickBuild...",
    regProgressDone:       "Complete",
    regressionFail:        "Regression check failed: {error}",
    ready:                 "Ready",
  };
  return _i18n[key] !== undefined ? _i18n[key] : key;
}
...
// addHtmlMessage: append an HTML message to the active result container.
// When the regression view is active (isRegressionMode === true, defined in
// sidepanel.js which loads after this file) results go to #regression-area;
// otherwise they go to #chat-area as before.
function addHtmlMessage(html) {
  const inRegMode = typeof isRegressionMode !== "undefined" && isRegressionMode;
  const box = document.getElementById(inRegMode ? "regression-area" : "chat-area");
...
// addSystemMessage: show a plain-text status line in the active container.
function addSystemMessage(text) {
  const inRegMode = typeof isRegressionMode !== "undefined" && isRegressionMode;
  if (inRegMode) {
    const box = document.getElementById("regression-area");
    const el = document.createElement("div");
    el.className = "msg msg-ai";
    el.textContent = text;
    box.appendChild(el);
...
// addMessage: wrapper for sidepanel.js message helpers
function addMessage(role, content) {
  if (role === "user") {
    if (typeof addUserMsg === "function") addUserMsg(content);
  } else {
    if (typeof addAiMsg === "function") addAiMsg(content);
  }
}
...
function formatQbBuilds(qbData, failVersion, passVersion, fixVersion, foundInVersion) {
  ...
  function pushBuild(b, version, role) {
    ...
    const entry = { version, build: bver, ci_master: ci, role, build_id: b.build_id || "", ci_build_id: b.ci_build_id || "", work_week: workWeek, revenue_pr: revenuePr, ci_builds: b.ci_builds || null };
  ...
  if (qbData && qbData.ok) {
    ...
    if (qbRows.length > 0) {
      ...
      for (const r of qbRows) {
        ...
        if (isGop) {
          let ciLabel;
          if (r.ci_builds && r.ci_builds.length > 0) {
            // Show range: first — last (both with links), tooltip on hover shows all subjects
            const first = r.ci_builds[0];
            const last  = r.ci_builds[r.ci_builds.length - 1];
            const allSubjects = r.ci_builds.map(cb => cb.change_subject).filter(Boolean).join("\n");
            const mkLink = (cb) => {
              const url     = `https://ubit-gfx.intel.com/build/${esc(cb.build_id)}`;
              const tooltip = allSubjects ? ` title="${esc(allSubjects)}"` : "";
              return `<a href="${url}" target="_blank"${tooltip}><strong>${esc(cb.version)}</strong></a>`;
            };
            ciLabel = r.ci_builds.length === 1
              ? mkLink(first)
              : `${mkLink(first)}<span style="color:#9ca3af"> - </span>${mkLink(last)}`;
          } else {
          ...
          html += `<td style="font-size:11px">${ciLabel}${cpNote}${bcNote}${prNote}</td>`;
...
function _setRegProgress(pct, text) {
  ...
  if (!container || !bar) return;  // null guard: element not in DOM
  if (pct < 0) {
...
function _bindReenterVersionsBtn(msgEl, data, fixV, foundInV, buildType) {
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      _setRegProgress(-1);  // Always hide progress bar after Re-enter flow
    }
  }, { once: true });
}
...
async function _historyLookup(hsdId, buildType) {
  try {
    ...
    return await r.json();          // {ok, cached, record?}
 ...
 * Each resolved build version (e.g. GOP "36" → gop-ci-main-36) is stored
 * individually so future HSD searches can reuse it without touching QB.
 */

async function _buildCacheLookup(versions, buildType) {
  /** Ask the bridge for cached build data for each version in the list. */
  try {
    ...
    return await r.json();   // {ok, found: {version: build_data}, missing: [...]}
  } catch { return { ok: false, found: {}, missing: versions }; }
}

async function _buildCacheSave(buildType, qbData) {
  /** Extract individual build objects from a QB result and persist them. */
  const entries = [];
...
function _assembleQbFromCache(failV, passV, fixV, foundInV, found) {
  /**
   * Try to reconstruct a complete qbData from the per-version cache.
   * Returns null if fail_version or pass_version are missing from cache.
   */
  if (failV && !found[failV]) return null;   // fail build not cached
  if (passV && !found[passV]) return null;   // pass build not cached
  return {
    ok: true,
...
/** Render a "from cache" banner above a result element. */
...
async function _checkRegression(buildType, forceRefresh = false) {
  ...
  try {
    ...
    // ── 1. Cache lookup (skip when forceRefresh) ────────────────────────────
    if (!forceRefresh) {
    ...
    if (qbData && qbData.ok) {
      ...
      // ── Persist to both caches (skip if data already came from build cache) ──
    ...
    isSending = false;
    _setRegProgress(-1);  // Always hide progress bar regardless of how the function exits
  }
}
...
async function showDriverHistoryPanel() {
  ...
  // Auto-select tab matching the last-used build type
  panel.querySelectorAll(".dh-tab").forEach(t =>
    t.classList.toggle("active", t.dataset.type === _lastRegBuildType)
  );

  await _renderHistoryList(_lastRegBuildType);
}
