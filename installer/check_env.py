...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self._row_results: dict = {}  # {index: (ok, msg)} — cached pass results
        self._fonts()
        self._build()
        self._center()
    ...
    def _run_checks(self):
        ...
        # Reset only rows that did not already pass
        for i, (badge, detail, install_btn, config_btn) in enumerate(self._rows):
            if self._row_results.get(i, (None,))[0] is True:
                continue  # already passed — leave UI as-is
        ...
        def worker():
            ...
            for i, (label, fn, _) in enumerate(CHECKS):
                if self._row_results.get(i, (None,))[0] is True:
                    # Already passed — reuse cached result, no need to re-run
                    ok, msg = self._row_results[i]
                else:
                    try:
                        ok, msg = fn()
                    except Exception as e:
